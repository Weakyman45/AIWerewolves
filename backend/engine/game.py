...
class WerewolfGame:
    def __init__(
        ...
        self.parallel_decisions = parallel_decisions
        self.decision_delay = decision_delay
        self.last_wolf_kill_target_id: Optional[str] = None
        self.last_wolf_kill_was_saved = False
        self.fake_seer_wolf_id: Optional[str] = None
        ...
        self._setup_players()
        self._assign_fake_seer_wolf()
        self._init_game_state()

    async def _collect_decisions(self, player_ids: List[str], decision_fn):
        if self.parallel_decisions:
            tasks = [decision_fn(pid) for pid in player_ids]
            return await asyncio.gather(*tasks)

        results = []
        for index, player_id in enumerate(player_ids):
            results.append(await decision_fn(player_id))
            if self.decision_delay > 0 and index < len(player_ids) - 1:
    ...
    def _assign_fake_seer_wolf(self):
        wolf_ids = [
            pid for pid, state in self.player_states.items()
            if state.role == Role.WEREWOLF
        ]
        self.fake_seer_wolf_id = wolf_ids[0] if wolf_ids else None
        for pid, agent in self.players.items():
            agent.can_fake_seer = pid == self.fake_seer_wolf_id
    ...
    def _get_agent_visible_state(self, player_id: str) -> dict:
        ...
        if viewer.role == Role.WEREWOLF:
            visible_state["wolf_last_kill_target_id"] = self.last_wolf_kill_target_id
            visible_state["wolf_last_kill_saved"] = self.last_wolf_kill_was_saved
    ...
    async def _run_round(self):
        ...
        if should_run_sheriff_election:
    ...
    async def _run_sheriff_election(self, round_record: RoundRecord):
        ...
        results = await self._collect_decisions(alive_player_ids, get_sheriff_election_decision)
        ...
        for player_id, decision in results:
            ...
            if decision.decision_type == "run":
                initial_candidates.append(player_id)
            else:
                initial_stay.append(player_id)
        ...
        sheriff_candidate_set = set()
        if seer_id:
            sheriff_candidate_set.add(seer_id)

        # 只允许本局指定的一名狼人悍跳；其他狼人必须留在警下做票型和站边。
        if self.fake_seer_wolf_id in alive_player_ids:
            sheriff_candidate_set.add(self.fake_seer_wolf_id)

        # 其他好人可随机上警，避免全员模板化留警下，也避免每局固定人数。
        for player_id in alive_player_ids:
            role = self.player_states[player_id].role
            if role in {Role.SEER, Role.WEREWOLF}:
                continue
            if random.random() < 0.7:
                sheriff_candidate_set.add(player_id)

        sheriff_candidates = [pid for pid in alive_player_ids if pid in sheriff_candidate_set]
        stay_in_election = [pid for pid in alive_player_ids if pid not in sheriff_candidate_set]
        for pid in alive_player_ids:
            self.player_states[pid].in_sheriff_election = pid in sheriff_candidate_set
    ...
    async def _run_sheriff_vote(self, candidates: List[str], round_record: RoundRecord, pk_round: int = 0):
        ...
        results = await self._collect_decisions(voters, get_sheriff_vote_decision)
    ...
    async def _run_night_phase(self, round_record: RoundRecord):
        ...
        self.last_wolf_kill_target_id = kill_victim
        self.last_wolf_kill_was_saved = bool(kill_victim and self._was_saved(kill_victim, round_record))

        if kill_victim and not self.last_wolf_kill_was_saved:
    ...
    async def _process_seer_check(self, round_record: RoundRecord):
        ...
        visible_state["night_unavailable_target_ids"] = [
            action.target_id
            for action in round_record.night_actions
            if action.action == NightAction.KILL and action.target_id
        ]
    ...
    async def _run_voting_round(self, round_record: RoundRecord, is_pk: bool = False):
        ...
        results = await self._collect_decisions(alive_player_ids, get_vote_decision)
